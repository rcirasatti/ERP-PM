...
@section('content')
    ...
    <div class="flex items-center justify-between mb-8">
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('penawaran.boq-template') }}" class="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-medium flex items-center space-x-2" title="Download template Excel untuk BoQ">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                </svg>
                <span>Template BoQ</span>
            </a>
            <!-- Create via BoQ Button -->
            <a href="{{ route('penawaran.create-boq') }}" class="px-6 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition font-medium flex items-center space-x-2" title="Buat penawaran dengan upload file Excel BoQ">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                </svg>
                <span>Buat via BoQ</span>
            </a>
            <!-- Create Manual Button -->
            <a href="{{ route('penawaran.create') }}" class="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-medium flex items-center space-x-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                </svg>
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
                        ...
                        @foreach ($penawaran as $item)
                            ...
                            <td class="px-6 py-4 text-sm whitespace-nowrap">
                                <div class="flex gap-2">
                                    <a href="{{ route('penawaran.show', $item->id) }}" class="text-blue-600 hover:text-blue-800 font-medium text-sm">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                        </svg>
                                    </a>
                                    <a href="{{ route('penawaran.edit', $item->id) }}" class="text-yellow-600 hover:text-yellow-800 font-medium text-sm">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @else
            <div class="p-8 text-center">
                ...
                <a href="{{ route('penawaran.create') }}" class="mt-4 inline-block px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
    ...
    <script>
        ...
        function deletePenawaran(id) {
            ...
            form.method = 'POST';
            form.action = '{{ route("penawaran.destroy", ":id") }}'.replace(':id', id);
