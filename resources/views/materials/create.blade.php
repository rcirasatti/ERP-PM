...
@section('title', 'Tambah Item Penawaran')
...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow-md p-8">
        <h2 class="text-2xl font-bold text-gray-900 mb-6">Tambah Item Penawaran Baru</h2>
        ...
        <form action="{{ route('material.store') }}" method="POST" class="space-y-6">
            ...
            <!-- Nama Item Penawaran -->
            <div>
                <label for="nama" class="block text-sm font-medium text-gray-700 mb-2">Nama Item Penawaran</label>
            ...
            <!-- Tipe Item Penawaran -->
            <div>
                <label for="type" class="block text-sm font-medium text-gray-700 mb-2">Tipe Item Penawaran</label>
                <select name="type" id="type" required
                    ...
                    @foreach ($types as $value => $label)
                        <option value="{{ $value }}" {{ old('type', 'BARANG') == $value ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" name="satuan" id="satuan" value="{{ old('satuan') }}" required
                    ...
                    placeholder="Contoh: Pcs, Box, Meter, Jam">
                @error('satuan')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div id="trackInventoryDiv" class="bg-blue-50 p-4 rounded-lg border border-blue-200">
                ...
                <p class="mt-2 text-xs text-gray-600">Centang untuk melacak stok item penawaran ini</p>
            </div>
