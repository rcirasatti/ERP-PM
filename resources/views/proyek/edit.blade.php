            ...
            <div class="bg-white rounded-lg shadow-md p-8">
                ...
                <form action="{{ route('proyek.update', $proyek->id) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="status" id="status" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('status') border-red-500 @enderror" required>
                            <option value="baru" @selected(old('status', $proyek->status) === 'baru')>Baru</option>
                            <option value="survei" @selected(old('status', $proyek->status) === 'survei')>Survei</option>
                            <option value="instalasi" @selected(old('status', $proyek->status) === 'instalasi')>Instalasi</option>
                            <option value="pengujian" @selected(old('status', $proyek->status) === 'pengujian')>Pengujian</option>
                            <option value="selesai" @selected(old('status', $proyek->status) === 'selesai')>Selesai</option>
                            <option value="bast" @selected(old('status', $proyek->status) === 'bast')>BAST</option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
